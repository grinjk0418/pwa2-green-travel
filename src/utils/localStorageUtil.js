...
// 숙박정보리스트 관련
  ...
  /**
   * 로컬스토리지에 스테이 페이지 번호 저장
   * @param {number} pageNo 
   */
  setStayPage: (pageNo) => {
    localStorage.setItem(KEY_LOCALSTORAGE_STAY_PAGE, JSON.stringify(pageNo));
  },
  /**
   * 로컬스토리지의 스테이 페이지 번호 반환
   * @returns {number} 페이지 번호
   */
  getStayPage: () => {
